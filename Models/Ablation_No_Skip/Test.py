...
### Define Final Upscale block       
class Upscale3DModel(nn.Module):
    def __init__(self):
        super(Upscale3DModel, self).__init__()        
 
        self.model = nn.Sequential(
            nn.Upsample(size=(48, 128, 128), mode='trilinear', align_corners=True),  # Upscale to 256x256x256      
        )
...
model.load_state_dict(torch.load(rf"{model_arch_name}.pth"))
...
# final test
test_input_dir = r"/Test_Dataset/Test_Data/Sino_Dataset_Test/Test_Sino"
test_output_dir = r"/Test_Dataset/Test_Data/Sino_Dataset_Test/Test_Att"
